...
<body class="bg-gray-100 flex">
    ...
    <main class="flex-1 ml-64 p-8">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()) : ?>
                        <tr class="text-center" id="row-<?php echo $row['id']; ?>">
                            ...
                            <td class="p-2 border">
                                <span id="name-display-<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['employee_name']); ?></span>
                                <input type="text" id="name-edit-<?php echo $row['id']; ?>" value="<?php echo htmlspecialchars($row['employee_name']); ?>" class="w-full p-1 border border-gray-300 hidden">
                            </td>
                            <td class="p-2 border">
                                <span id="username-display-<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['username']); ?></span>
                                <input type="text" id="username-edit-<?php echo $row['id']; ?>" value="<?php echo htmlspecialchars($row['username']); ?>" class="w-full p-1 border border-gray-300 hidden">
                            </td>
                            <td class="p-2 border">
                                <span id="mobile-display-<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['mobile_no']); ?></span>
                                <input type="text" id="mobile-edit-<?php echo $row['id']; ?>" value="<?php echo htmlspecialchars($row['mobile_no']); ?>" class="w-full p-1 border border-gray-300 hidden">
                            </td>
                            <td class="p-2 border">
                                <input type="password" id="password-edit-<?php echo $row['id']; ?>" placeholder="New Password (Optional)" class="w-full p-1 border border-gray-300 hidden">
                            </td>
                            <td class="p-2 border flex justify-center space-x-2">
                                <button onclick="enableEdit(<?php echo $row['id']; ?>)" class="bg-blue-500 text-white px-3 py-1 rounded">Edit</button>
                                <button onclick="saveEdit(<?php echo $row['id']; ?>)" class="bg-green-500 text-white px-3 py-1 rounded hidden" id="save-btn-<?php echo $row['id']; ?>">Save</button>
        ...
        <div class="mt-4 flex justify-center space-x-2">
            <?php if ($page > 1) : ?>
                <a href="?search=<?php echo $search; ?>&page=<?php echo $page - 1; ?>" class="px-4 py-2 bg-gray-300 rounded">Previous</a>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                <a href="?search=<?php echo $search; ?>&page=<?php echo $i; ?>" class="px-4 py-2 <?php echo $i == $page ? 'bg-blue-500 text-white' : 'bg-gray-300'; ?> rounded">
                    <?php echo $i; ?>
                </a>
            <?php endfor; ?>

            <?php if ($page < $totalPages) : ?>
                <a href="?search=<?php echo $search; ?>&page=<?php echo $page + 1; ?>" class="px-4 py-2 bg-gray-300 rounded">Next</a>
            <?php endif; ?>
        </div>
    </main>
</body>
