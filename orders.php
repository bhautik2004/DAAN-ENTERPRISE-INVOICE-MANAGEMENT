        ...
        <?php
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                WHERE id = ?";
                                ...
                                // Rest of your item processing code remains the same...
                                // [Keep the existing item update/insert/delete logic here]
                        ...
                        try {
                            ...
                            if ($stmt->execute()) {
                                $conn->commit();
                                echo "<script>showMessage('Invoice deleted successfully!', 'success'); setTimeout(() => { window.location.reload(); }, 1500);</script>";
                            } else {
...
<?php endif; ?>
                                    ...
                                    <div id="items-edit-<?php echo $row['id']; ?>" class="hidden space-y-2 text-xs">
                                        ...
                                        <div class="item-row grid grid-cols-5 gap-2">
                                            ...
                                            <input type="number" name="price[]" value="<?php echo $item['price']; ?>"
                                                class="price-input editable p-1 border rounded"
                                                onchange="calculateRowTotal(this,                                                                                                                                                                                                                                                                                                                                     <?php echo $row['id']; ?>)"
                                ...
                                <td class="p-2 border">
                                    ₹<span
                                        id="total-display-<?php echo $row['id']; ?>"><?php echo number_format($row['total_amount'], 2); ?></span>
    ...
    document.querySelectorAll('.column-toggle').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            document.querySelectorAll(`td:nth-child(${columnIndex}), th:nth-child(${columnIndex})`)
                .forEach(
                    cell => {
    ...
    function addItemRow(invoiceId) {
        ...
        <input type="number" name="price[]" value="<?php echo $products[0]['price'] ?? 0; ?>"
               class="editable p-1 border rounded price-input"
               onchange="calculateRowTotal(this, ${invoiceId})"
               min="0" step="0.01" required>
    ...
    // Calculate total for the entire invoice
    function calculateInvoiceTotal(invoiceId) {
        ...
        // Get advanced payment value
        const advancedPayment = parseFloat(document.querySelector(`#form_${invoiceId} input[name="advanced_payment"]`)
            .value) || 0;

        // Update display and hidden input
        document.getElementById(`total-display-${invoiceId}`).textContent = total.toFixed(2);
        document.getElementById(`total-input-${invoiceId}`).value = total.toFixed(2);

        // Calculate balance (if needed)
        const balance = total - advancedPayment;
